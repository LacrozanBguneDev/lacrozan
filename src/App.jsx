...
import { 
    ...
    Scale, FileText, ChevronLeft, CornerDownRight, Video
} from 'lucide-react';
...
// 2. Kompresi Gambar ke Base64 (Hanya untuk Foto sesuai request)
...
// 3. Upload File API (Untuk Video & Audio - FITUR BARU)
const uploadFileToAPI = async (file) => {
    const formData = new FormData();
    formData.append('file', file);
    try {
        const response = await fetch("https://api-faa.my.id/faa/tourl", { method: 'POST', body: formData });
        ...
        return url;
    } catch (error) { console.error("API Upload Error:", error); throw error; }
};
...
const ImageWithRetry = ({ src, alt, className, onClick }) => {
    ...
    if (error) return <div className={`bg-gray-100 dark:bg-gray-700 flex flex-col items-center justify-center text-gray-400 dark:text-gray-500 ${className}`} style={{minHeight: '200px'}}><ImageOff size={24} className="mb-2 opacity-50"/><p className="text-[10px]">Gagal Memuat</p></div>;
    return <img src={src} alt={alt} className={className} onClick={onClick} onError={() => setError(true)} loading="lazy"/>;
};

// --- MEDIA GRID SYSTEM (FITUR BARU GRID) ---
const MediaGrid = ({ mediaUrls, onImageClick }) => {
    ...
    // Grid Logic
    if (count === 1) {
        return <div className="mb-4 rounded-2xl overflow-hidden aspect-video" onClick={() => onImageClick(0)}><img src={mediaUrls[0]} className="w-full h-full object-cover cursor-pointer hover:scale-105 transition duration-500"/></div>;
    }
    if (count === 2) {
        return <div className="mb-4 grid grid-cols-2 gap-1 rounded-2xl overflow-hidden aspect-video">{mediaUrls.map((url, i) => (<div key={i} className="relative h-full" onClick={() => onImageClick(i)}><img src={url} className="w-full h-full object-cover cursor-pointer hover:brightness-90 transition"/></div>))}</div>;
    }
    if (count === 3) {
        return <div className="mb-4 grid grid-cols-2 gap-1 rounded-2xl overflow-hidden aspect-square"><div className="col-span-2 h-[60%]" onClick={() => onImageClick(0)}><img src={mediaUrls[0]} className="w-full h-full object-cover cursor-pointer hover:brightness-90 transition"/></div><div className="h-[40%]" onClick={() => onImageClick(1)}><img src={mediaUrls[1]} className="w-full h-full object-cover cursor-pointer hover:brightness-90 transition"/></div><div className="h-[40%]" onClick={() => onImageClick(2)}><img src={mediaUrls[2]} className="w-full h-full object-cover cursor-pointer hover:brightness-90 transition"/></div></div>;
    }
    // 4 or more
    return (
        <div className="mb-4 grid grid-cols-2 gap-1 rounded-2xl overflow-hidden aspect-square">
            {mediaUrls.slice(0, 4).map((url, i) => (
                <div key={i} className="relative h-full" onClick={() => onImageClick(i)}>
                    ...
                    {i === 3 && count > 4 && <div className="absolute inset-0 bg-black/50 flex items-center justify-center text-white font-bold text-xl cursor-pointer">+{count - 4}</div>}
                </div>
            ))}
        </div>
    );
...
// --- RENDER MARKDOWN (HASHTAG SUPPORT) ---
const RenderMarkdown = ({ text, onHashtagClick }) => {
    ...
    return (
        <div className="text-gray-800 dark:text-gray-200 leading-relaxed text-sm whitespace-pre-wrap break-words">
            {parts.map((part, i) => {
                if (part.startsWith('#')) return <span key={i} onClick={(e) => { e.stopPropagation(); onHashtagClick(part); }} className="text-sky-500 font-bold cursor-pointer hover:underline">{part}</span>;
                if (part.match(/^https?:\/\//)) return <a key={i} href={part} target="_blank" rel="noopener noreferrer" className="text-sky-600 hover:underline break-all" onClick={e=>e.stopPropagation()}>{part}</a>;
                return part;
            })}
        </div>
    );
...
// ==========================================
// BAGIAN 4: DASHBOARD DEVELOPER & FEATURES
// ==========================================
...
// ==========================================
// BAGIAN 5: LAYAR OTENTIKASI & LEGAL
// ==========================================
...
const OnboardingScreen = ({ onComplete, user }) => {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault(); if (!username.trim()) return alert("Username wajib diisi!"); setLoading(true);
        try { await setDoc(doc(db, getPublicCollection('userProfiles'), user.uid), { username: username.trim(), email: user.email, uid: user.uid, photoURL: user.photoURL || '', createdAt: serverTimestamp(), following: [], followers: [], savedPosts: [], lastSeen: serverTimestamp() }); onComplete(); } catch (error) { alert("Gagal menyimpan data: " + error.message); } finally { setLoading(false); }
    };
    return (
    ...
    return (
        <div className="min-h-screen bg-white dark:bg-gray-900 pb-24 pt-20 px-6 max-w-2xl mx-auto animate-in fade-in">
            ...
            <div className="space-y-8">
                <section><h2 className="text-lg font-bold text-gray-800 dark:text-white mb-3 flex items-center gap-2"><Lock size={18} className="text-sky-500"/> Kebijakan Privasi</h2><div className="bg-gray-50 dark:bg-gray-800 p-5 rounded-2xl text-sm text-gray-600 dark:text-gray-300 leading-relaxed border border-gray-100 dark:border-gray-700"><p className="mb-3">Di {APP_NAME}, privasi Anda adalah prioritas kami. Kami mengumpulkan data minimal yang diperlukan untuk fungsionalitas aplikasi.</p><ul className="list-disc pl-5 space-y-1 mb-3"><li><strong>Data Akun:</strong> Nama, Email, Foto Profil (via Google Login).</li><li><strong>Konten:</strong> Postingan, Komentar, dan Pesan yang Anda buat.</li><li><strong>Aktivitas:</strong> Log interaksi seperti Like dan Follow untuk personalisasi.</li></ul><p>Kami tidak menjual data Anda ke pihak ketiga. Data disimpan aman menggunakan infrastruktur Google Cloud (Firebase).</p></div></section>
                <section><h2 className="text-lg font-bold text-gray-800 dark:text-white mb-3 flex items-center gap-2"><FileText size={18} className="text-purple-500"/> Ketentuan Layanan</h2><div className="bg-gray-50 dark:bg-gray-800 p-5 rounded-2xl text-sm text-gray-600 dark:text-gray-300 leading-relaxed border border-gray-100 dark:border-gray-700"><p className="mb-3">Dengan menggunakan aplikasi ini, Anda setuju untuk:</p><ul className="list-disc pl-5 space-y-1"><li>Tidak memposting konten ilegal, pornografi, atau ujaran kebencian.</li><li>Saling menghormati antar pengguna.</li><li>Tidak melakukan spam atau aktivitas bot.</li></ul><p className="mt-3 text-rose-500 font-bold text-xs">Pelanggaran dapat mengakibatkan pemblokiran akun permanen.</p></div></section>
                <section><h2 className="text-lg font-bold text-gray-800 dark:text-white mb-3 flex items-center gap-2"><Info size={18} className="text-emerald-500"/> Tentang Kami</h2><div className="bg-gray-50 dark:bg-gray-800 p-5 rounded-2xl text-sm text-gray-600 dark:text-gray-300 leading-relaxed border border-gray-100 dark:border-gray-700"><p>BguneNet dikembangkan oleh <strong>Irham Andika</strong> (Siswa SMPN 3 Mentok) sebagai platform sosial media alternatif yang ringan, cepat, dan berfokus pada komunitas lokal & global.</p><p className="mt-2 text-xs text-gray-400">Versi Aplikasi: 25.0 (Ultimate Build)</p></div></section>
            </div>
        </div>
    );
};
            ...
            <div className="bg-white dark:bg-gray-800 rounded-3xl shadow-sm border border-gray-100 dark:border-gray-700 overflow-hidden">
                {sortedUsers.map((u, index) => (
                    <div key={u.uid} className={`flex items-center p-4 border-b border-gray-50 dark:border-gray-700 last:border-0 ${index < 3 ? 'bg-gradient-to-r from-yellow-50/50 to-transparent dark:from-yellow-900/10' : ''}`}>
                        ...
                        <div className="relative mr-3"><img src={u.photoURL || APP_LOGO} className="w-10 h-10 rounded-full object-cover border border-gray-200 dark:border-gray-600"/>{index === 0 && <Crown size={14} className="absolute -top-1 -right-1 text-yellow-500 fill-yellow-500"/>}</div>
...
// --- POST ITEM (GRID FOTO & REPLY SYSTEM & GUEST CHECK) ---
const PostItem = ({ post, currentUserId, profile, handleFollow, goToProfile, isMeDeveloper, isGuest, onRequestLogin, onHashtagClick }) => {
    ...
    const handleComment = async (e) => {
        ...
        try {
            ...
            if (post.userId !== currentUserId && (!replyingTo || replyingTo.userId !== post.userId)) sendNotification(post.userId, 'comment', `mengomentari postingan Anda`, profile, post.id);
            setNewComment('');
            setReplyingTo(null);
    ...
    const sharePost = async () => { try { await navigator.clipboard.writeText(`${window.location.origin}?post=${post.id}`); alert('Link Disalin! (Bisa dibuka tanpa login)'); } catch (e) { alert('Gagal menyalin link'); } };
    ...
    return (
        <div className="bg-white dark:bg-gray-800 rounded-[2rem] p-5 mb-6 shadow-[0_10px_40px_-15px_rgba(0,0,0,0.05)] border border-gray-100 dark:border-gray-700 relative overflow-hidden group transition hover:shadow-lg">
            ...
            {isMeme && <div className="absolute top-4 right-4 bg-yellow-100 dark:bg-yellow-900/30 text-yellow-700 dark:text-yellow-400 text-[10px] font-bold px-3 py-1 rounded-full border border-yellow-200 dark:border-yellow-800 flex items-center z-10"><Laugh size={10} className="mr-1"/> MEME</div>}
            ...
            {isEditing ? (
                ...
                <>
                    ...
                    <div className="text-sm text-gray-600 dark:text-gray-300 mb-4 leading-relaxed"><RenderMarkdown text={displayText} onHashtagClick={onHashtagClick} />{isLongText && <button onClick={() => setIsExpanded(!isExpanded)} className="text-sky-600 font-bold text-xs ml-1 hover:underline inline-block mt-1">{isExpanded ? 'Sembunyikan' : 'Baca Selengkapnya'}</button>}</div>
                ...
                <div className="mt-5 pt-4 border-t border-gray-100 dark:border-gray-700 animate-in fade-in">
                    <div className="max-h-60 overflow-y-auto space-y-3 mb-3 custom-scrollbar pr-1">
                        {comments.length === 0 ? <p className="text-xs text-center text-gray-400">Belum ada komentar.</p> : comments.map(c => ( 
                            <div key={c.id} className={`group ${c.parentId ? 'ml-8' : ''}`}>
                                {c.parentId && <div className="text-[10px] text-gray-400 mb-1 flex items-center gap-1"><CornerDownRight size={10}/> Balasan untuk @{c.replyToUsername}</div>}
                                <div className="bg-gray-50 dark:bg-gray-750 dark:bg-gray-900 p-3 rounded-xl text-xs flex justify-between items-start">
                                    <div className="flex-1">
                                        <div className="flex justify-between">
                                            <span className="font-bold text-gray-800 dark:text-gray-200 mr-1">{c.username}</span>
                                            <button onClick={() => setReplyingTo({id: c.id, username: c.username})} className="text-[10px] text-sky-500 font-bold hover:underline opacity-0 group-hover:opacity-100 transition">Balas</button>
                                        </div>
                                        <span className="text-gray-600 dark:text-gray-400">{c.text}</span>
                                    </div>
                                    {(currentUserId === c.userId || isMeDeveloper) && <button onClick={() => handleDeleteComment(c.id)} className="text-gray-400 hover:text-red-500 opacity-0 group-hover:opacity-100 transition ml-2">{isMeDeveloper && currentUserId !== c.userId ? <ShieldAlert size={12}/> : <Trash size={12}/>}</button>}
                                </div>
                            </div> 
                        ))}
                    </div>
                    ...
                    <form onSubmit={handleComment} className="relative">
                        ...
                        <div className="flex gap-2">
                            <input value={newComment} onChange={(e) => setNewComment(e.target.value)} placeholder={isGuest ? "Login untuk komentar..." : replyingTo ? "Tulis balasan..." : "Tulis komentar..."} disabled={isGuest} className={`flex-1 bg-gray-100 dark:bg-gray-700 dark:text-white rounded-xl px-4 py-2.5 text-xs outline-none focus:ring-2 focus:ring-sky-200 ${replyingTo ? 'rounded-tl-none' : ''}`}/>
                            <button type="submit" disabled={!newComment.trim() || isGuest} className="p-2.5 bg-sky-500 text-white rounded-lg shadow-md hover:bg-sky-600 disabled:opacity-50"><Send size={16}/></button>
                        </div>
                    </form>
                </div>
            )}
...
// --- CREATE POST (UPDATE: API VIDEO & BASE64 FOTO & HAPUS SHORT) ---
const CreatePost = ({ setPage, userId, username, onSuccess }) => {
    const [form, setForm] = useState({ title: '', content: '', files: [], url: '', isAudio: false });
    ...
    const handleFileChange = (e) => {
        ...
        if (selectedFiles.length > 0) {
            ...
            setForm({ ...form, files: selectedFiles, isAudio: isAudio, url: '' });
        }
    };
        ...
        try {
            ...
            if (form.files.length > 0) {
                ...
                if (firstFile.type.startsWith('image')) {
                    ...
                    // Video & Audio Pindah ke API
                    const url = await uploadFileToAPI(firstFile);
                    mediaUrls.push(url);
            ...
            const ref = await addDoc(collection(db, getPublicCollection('posts')), {
                ...
                likes: [], 
                commentsCount: 0, 
                isShort: false, // Shorts dimatikan total
                category: category, 
        ...
        <div className="max-w-xl mx-auto p-4 pb-24">
            <div className="bg-white dark:bg-gray-800 rounded-[2rem] p-6 shadow-xl border border-sky-50 dark:border-gray-700 relative overflow-hidden mt-4">
                ...
                <form onSubmit={submit} className="space-y-4">
                    ...
                    <textarea value={form.content} onChange={e=>setForm({...form, content:e.target.value})} placeholder="Ceritakan sesuatu... Gunakan #hashtag" rows="4" className="w-full p-3 bg-gray-50 dark:bg-gray-700 dark:text-white rounded-xl text-sm outline-none focus:ring-2 focus:ring-sky-200 transition resize-none"/>
                    ...
                    <div className="flex gap-2 overflow-x-auto no-scrollbar">
                        <label className={`flex items-center px-4 py-3 rounded-xl border cursor-pointer flex-1 whitespace-nowrap transition ${form.files.length > 0 && !form.isAudio ?'bg-sky-50 border-sky-200 text-sky-600':'border-gray-200 dark:border-gray-600 text-gray-500 dark:text-gray-400'}`}>
                         ...
                         <label className={`flex items-center px-4 py-3 rounded-xl border cursor-pointer flex-1 whitespace-nowrap transition ${form.files.length > 0 && form.files[0].type.startsWith('video') ?'bg-pink-50 border-pink-200 text-pink-600':'border-gray-200 dark:border-gray-600 text-gray-500 dark:text-gray-400'}`}>
                            <Video size={18} className="mr-2"/>
                            <span className="text-xs font-bold">Video</span>
                            <input type="file" className="hidden" accept="video/*" onChange={handleFileChange} disabled={loading}/>
                        </label>
                        <label className={`flex items-center px-4 py-3 rounded-xl border cursor-pointer flex-1 whitespace-nowrap transition ${form.isAudio ?'bg-purple-50 border-purple-200 text-purple-600':'border-gray-200 dark:border-gray-600 text-gray-500 dark:text-gray-400'}`}>
...
// --- HOME SCREEN ---
const HomeScreen = ({ currentUserId, profile, allPosts, handleFollow, goToProfile, newPostId, clearNewPost, isMeDeveloper, isGuest, onRequestLogin, filterHashtag, clearFilter }) => {
    ...
    const filteredPosts = useMemo(() => {
        let p = allPosts;
        if (filterHashtag) { p = p.filter(post => post.content && post.content.includes(filterHashtag)); }
        return p;
    }, [allPosts, filterHashtag]);

    useEffect(() => {
        if (allPosts.length === 0) { setLoadingFeed(false); return; }
        let basePosts = filteredPosts.filter(p => !p.isShort); // Hapus Short dari feed
        let pinnedPost = null;
        if (newPostId) {
        ...
        let processedPosts = [];
        if (sortType === 'latest' || filterHashtag) processedPosts = basePosts.sort((a, b) => (b.timestamp?.toMillis || 0) - (a.timestamp?.toMillis || 0));
    ...
    }, [filteredPosts, sortType, newPostId, filterHashtag]); 
    ...
    return (
        <div className="max-w-lg mx-auto pb-24 px-4">
             {filterHashtag && (
                <div className="bg-sky-500 text-white p-4 rounded-2xl mb-6 flex justify-between items-center shadow-lg mt-4 animate-in fade-in">
                    <div><p className="text-xs opacity-80">Menampilkan hasil untuk:</p><h2 className="font-black text-xl">{filterHashtag}</h2></div>
                ...
                <>
                    {visiblePosts.map(p => (
                        <div key={p.id} className={p.id === newPostId ? "animate-in slide-in-from-top-10 duration-700" : ""}>
                            ...
                            <PostItem post={p} currentUserId={currentUserId} currentUserEmail={profile?.email} profile={profile} handleFollow={handleFollow} goToProfile={goToProfile} isMeDeveloper={isMeDeveloper} isGuest={isGuest} onRequestLogin={onRequestLogin} onHashtagClick={(tag) => clearFilter ? clearFilter(tag) : {}}/>
                        </div>
                    ))}
...
const App = () => {
    ...
    const [hashtagFilter, setHashtagFilter] = useState(null); // State Hashtag
    ...
    useEffect(() => onAuthStateChanged(auth, async (u) => { 
        if(u) { 
            ...
            if (!userDoc.exists()) { setShowOnboarding(true); } else { await updateDoc(doc(db, getPublicCollection('userProfiles'), u.uid), { lastSeen: serverTimestamp() }).catch(()=>{}); }
        } else { setUser(null); setProfile(null); } 
    }), []);

    useEffect(() => { 
        if(user) {
            const unsubP = onSnapshot(doc(db, getPublicCollection('userProfiles'), user.uid), s => { if(s.exists()) { setProfile({...s.data(), uid:user.uid, email:user.email}); if (showOnboarding) setShowOnboarding(false); } });
    ...
    const handleHashtag = (tag) => { setHashtagFilter(tag); setPage('home'); window.scrollTo(0,0); };
            ...
            <div className={`min-h-screen bg-[#F0F4F8] dark:bg-gray-900 font-sans text-gray-800 dark:text-gray-100 transition-colors duration-300`}>
                ...
                {page!=='legal' && (
                    <nav className="fixed bottom-6 left-1/2 transform -translate-x-1/2 bg-white/90 dark:bg-gray-800/90 backdrop-blur-xl border border-white/50 dark:border-gray-700 rounded-full px-6 py-3 shadow-2xl shadow-sky-100/50 dark:shadow-none flex items-center gap-6 z-40">
                        <NavBtn icon={Home} active={page==='home'} onClick={()=>{setPage('home'); setHashtagFilter(null);}}/>
